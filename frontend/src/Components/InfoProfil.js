import React from "react";



function InfoProfil(props) {
    console.log(props)    

    return (

        

        <table className="min-w-full leading-normal">

            <tbody>
                <tr className="grid grid-cols-2">
                    <td className="p-5 border-b border-gray-200 bg-white text-sm col-span-2 md:col-span-1">
                        <div className="block md:flex flex-shrink-0 w-32 h-32">
                            <img className="w-full h-full rounded-full" src="data:image/jpeg;base64," alt="photo_profil"></img>
                        </div>
                    </td>
                    <td className="px-10 pt-4 col-span-2 md:col-span-1">
                        <div className="p-1">
                            <input placeholder="a"></input>
                        </div>
                        <div className="p-1">
                            <input placeholder="Nom" disabled></input>
                        </div>
                        <div className="p-1">
                            <input placeholder="Prénom" disabled></input>
                        </div>
                        <div className="p-1">
                            <input placeholder="DatedeNaissance" disabled></input>
                        </div>
                    </td>
                </tr>
                <tr className="grid grid-cols-2">
                    <td  className="py-4 px-10 border-b border-gray-200 bg-white text-sm col-span-2 md:col-span-1">
                        <div className="p-1">
                            <input placeholder="Genre" disabled></input>
                        </div>
                  
                        <div className="p-1">
                            <input placeholder="Tel" disabled></input>
                        </div>
                  
                        <div className="p-1">
                            <input placeholder="adresse" disabled></input>
                        </div>
                  
                        <div className="p-1">
   
                            <input placeholder="courriel" disabled></input>
                        </div>
                                         
                        <div className="p-1">
                            <input placeholder="ID legal" disabled></input>
                        </div>
                  
                        <div className="p-1">
                            <input placeholder="Nationalité" disabled></input>
                        </div>
                        </td>
                        <td className="py-4 px-10 border-b border-gray-200 bg-white text-sm col-span-2 md:col-span-1">
                        <div className="p-1">
                            <input placeholder="Facebook" disabled></input>
                        </div>
                   
                        <div className="col-span-2 md:col-span-1 p-1">
                            <input placeholder="Instagral" disabled></input>
                        </div>
                   
                        <div className="col-span-2 md:col-span-1 p-1">
                            <input placeholder="Twitter" disabled></input>
                        </div>
                    </td>
                </tr>
                <tr className="items-center">
                    <div className="items-center">
                    <button className="h-10 text-center w-24 text-xs p-2 my-15 mx-2 bg-orange-400 text-orange-700 font-medium rounded border border-orange-700 hover:border-orange-700 hover:bg-orange-600 hover:text-orange-400 hover:shadow-xl">Modifier</button>
                    
                    <button className="h-10 text-center w-24 text-xs p-2 my-15 mx-2 bg-red-400 text-red-700 font-medium rounded border border-red-700 hover:border-red-700 hover:text-red-400 hover:bg-red-600 hover:shadow-xl">Supprimer</button>
                    </div>
                </tr>
                {/* <tr className="grid grid-cols-2">
                                       
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm" >
                        <input className="disabled">ale</input>
                        <input className="disabled">ale</input>
                        <input className="disabled">ale</input>
                        <input className="disabled">ale</input>

                    </td>
                </tr>
                <tr>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <div className="flex items-start m-3"> <p className="text-gray-900">
                            Jan 21, 2020
                                    </p></div>
                    </td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <div className="flex items-start m-3">
                            <button class="bg-teal-500 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded">
                                Info
                              </button>
                        </div>
                    </td>

                </tr> */}

            </tbody>

        </table>

    )
}

export default InfoProfil;